...
async function persist() {
  ...
  const toSave = await Promise.all(
    connections.value.map(async (conn) => {
      if (conn.password) {
        const encrypted: string = await invoke('encrypt_password', { plaintext: conn.password })
...
async function loadConnections() {
  ...
  if (raw) {
    ...
    connections.value = await Promise.all(
      raw.map(async (conn) => {
        if (conn.password) {
          try {
            const decrypted: string = await invoke('decrypt_password', { encrypted: conn.password })
  ...
  activeConnectionId.value = (await s.get<string>('activeConnectionId')) ?? null
  loaded.value = true
}
